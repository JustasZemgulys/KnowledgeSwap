...
class _TestAssignmentDetailScreenState extends State<TestAssignmentDetailScreen> {
  ...
  Future<void> _fetchAssignedUsers() async {
    ...
    try {
      final url = Uri.parse('https://juszem1-1.stud.if.ktu.lt/get_assignment_users.php?assignment_id=${widget.assignment['id']}');
  ...
  Future<void> _updateAssignedUsers(List<dynamic> updatedUserList) async {
    ...
    try {
      final url = Uri.parse('https://juszem1-1.stud.if.ktu.lt/add_users_to_assignment.php');
  ...
  Future<void> _deleteAssignment() async {
    ...
    if (confirmed == true) {
      try {
        final url = Uri.parse('https://juszem1-1.stud.if.ktu.lt/delete_test_assignment.php');
        final response = await http.post(
          url,
          headers: {'Content-Type': 'application/json'},
          body: json.encode({
